...
/* FUNCIONES EXAMEN DE PRÁCTICAS*/

int InsertQuickSort(int *tabla, int ip, int iu, int L){
	if(!tabla || ip>iu || ip<0 || L<0) return ERR;
	int tam = iu - ip + 1;
	int ob = 0, pos;
	if(tam <= L){
		ob += InsertSort(tabla, ip, iu);
	}else{
	
		if(ip==iu)
			return OK;
		ob+= partir(tabla, ip, iu, &pos, 0);
		if(ip < pos-1)
			ob+=InsertQuickSort(tabla, ip, pos-1, L);
		if(iu > pos+1)
		 	ob+=InsertQuickSort(tabla, pos+1, iu, L);
		 	
	}
	return ob;
}

int InsertQuickSort100(int *tabla, int ip, int iu, int arg_inutil){
	if(!tabla || ip>iu || ip<0) return ERR;
	int ob = InsertQuickSort(tabla, ip, iu, 100);
	return ob;
}

int InsertQuickSort200(int *tabla, int ip, int iu, int arg_inutil){
	if(!tabla || ip>iu || ip<0) return ERR;
	int ob = InsertQuickSort(tabla, ip, iu, 200);
	return ob;
}

int InsertQuickSort300(int *tabla, int ip, int iu, int arg_inutil){
	if(!tabla || ip>iu || ip<0) return ERR;
	int ob = InsertQuickSort(tabla, ip, iu, 300);
	return ob;
}

int InsertQuickSort400(int *tabla, int ip, int iu, int arg_inutil){
	if(!tabla || ip>iu || ip<0) return ERR;
	int ob = InsertQuickSort(tabla, ip, iu, 400);
	return ob;
}
