...
int main(int argc, char** argv)
{
  ...
  for(i = 1; i < argc ; i++) {
	...
	/* calculamos los tiempos, MERGESORT */
	ret = genera_tiempos_ordenacion(mergesort, "mergesort.log" ,num_min, num_max,incr, n_perms, -1);
	if (ret == ERR ) { /* ERR_TIME debera ser un numero negativo */
		printf("Error en la funcion Time_Ordena\n");
		exit(-1);
	}
	/*tiempos para QUICKSORT con PIVOTE IP */
	ret = genera_tiempos_ordenacion(quicksort, "quicksort_ip.log",num_min, num_max,incr, n_perms, 0);
	if (ret == ERR ) { /* ERR_TIME debera ser un numero negativo */
		printf("Error en la funcion Time_Ordena\n");
		exit(-1);
	}
	/*tiempos para QUICKSORT con PIVOTE MEDIO_AVG*/
	ret = genera_tiempos_ordenacion(quicksort, "quicksort_medio_avg.log",num_min, num_max,incr, n_perms, 1);
	if (ret == ERR ) { /* ERR_TIME debera ser un numero negativo */
		printf("Error en la funcion Time_Ordena\n");
		exit(-1);
	}
	/*tiempos para QUICKSORT con PIVOTE MEDIO_STAT*/
	ret = genera_tiempos_ordenacion(quicksort, "quicksort_medio_stat.log",num_min, num_max,incr, n_perms, 2);
	if (ret == ERR ) { /* ERR_TIME debera ser un numero negativo */
		printf("Error en la funcion Time_Ordena\n");
		exit(-1);
	}
